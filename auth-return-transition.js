...
let fromAdmin=params.get('from')==='admin';
if(!fromAdmin&&document.referrer){try{const r=new URL(document.referrer);fromAdmin=r.origin===location.origin&&/\/admin\.html$/i.test(r.pathname)}catch{}}
if(!fromAdmin)return;
...
function clear(removeQuery=true){document.getElementById('adminReturnTransition')?.remove();document.getElementById('adminReturnTransitionStyle')?.remove();if(removeQuery&&params.has('from')){const u=new URL(location.href);u.searchParams.delete('from');history.replaceState(null,'',u.pathname+(u.search||'')+u.hash)}}
