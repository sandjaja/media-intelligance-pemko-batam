...
export type PrintWarningSignal={
  engine:string;
  generatedAt:string;
  printArticleId:number;
  warningScore:number;
  warningLevel:WarningLevel;
  candidateAlert:boolean;
...
async function compute(client:PoolClient,article:any):Promise<PrintWarningSignal>{
  ...
  const warningScore=clamp(score);
  const warningLevel=level(warningScore);
  const evidenceSourceCount=evidence.rowCount||0;
  const phaseConfidence=String(p.confidence||'LOW').toUpperCase();
