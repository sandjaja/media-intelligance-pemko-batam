...
export const CLASSIFICATION_VERSION='article-opd-v14-20260916';
...
export async function analyzeArticle(pool:Pool,articleId:string){
 ...
 // Human decisions are authoritative. A manual PENDUKUNG must never be promoted by reanalysis.
 let news=await getManualNewsClassification(pool,articleId);
 let routing:any=null;
 if(!news||news.classification==='UTAMA'){
  // Master Classification is the single source of AUTO routing. Run it exactly once here.
 ...
 // MANUAL UTAMA is valid only when routing can produce a Primary OPD (normally via manual keyword correction).
 ...
 if(!opdId){
  ...
  return{articleId,newsClassification:'PENDUKUNG',newsClassificationSource:'AUTO',newsClassificationReason:'no Primary OPD routing',newsClassificationSignals:[],classificationVersion:CLASSIFICATION_VERSION,opdId:null,supportingOpdIds:[],districtId:null,uptdId:null,uptdName:null,uptdMatches:0,taxonomyId:null,taxonomyName:null,taxonomyScore:0,issueId:null,issueMatchScore:0,issueAssignmentSource:null,risk:null};
 }
