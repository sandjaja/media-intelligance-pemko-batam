...
const FALLBACK_RULES: CategoryRule[] = [
  ...
  { name: 'Infrastruktur & Transportasi', terms: ['jalan','jembatan','pelabuhan','transportasi','kemacetan','macet','drainase','lampu jalan','infrastruktur','simpang','kendaraan','akses jalan','jalan tergenang','jalan terendam','genangan jalan','lalu lintas','lalulintas','arus lalu lintas','lalu lintas tersendat','lalulintas tersendat','tersendat'] },
...
function fallback(title: string, summary: string, body: string, operatorMatches: OperatorMatch[]) {
  ...
  const scored = FALLBACK_RULES.map(rule => {
    ...
    if (rule.generic) {
      let genericHits = 0;
      for (const termRaw of rule.generic) {
        const term = norm(termRaw);
        if (count(title, term) + count(summary, term) + count(body, term)) {
          genericHits++;
          evidence.push(`${termRaw}: konteks generik`);
        }
      }
      if (score > 0) score += Math.min(4, genericHits);
    }
...
export function classifyIssueWithTaxonomy(titleRaw: string, summaryRaw: string, bodyRaw: string, mappings: TaxonomyMapping[], operatorMatches: OperatorMatch[]) {
  ...
  for (const row of mappings || []) {
    const name = String(row.taxonomy_name || '').trim();
    if (!name) continue;
  ...
  return {
    ...
    categoryEvidence: {
      ...
      candidates: ranked.slice(0, 4).map(x => ({ ...x, score: Number(x.score.toFixed(2)) }))
    }
  };
}