...
import { linkOnlineArticleToIssueMonitors } from './issue-monitor-matcher.js';
...
export async function analyzeArticle(pool:Pool,articleId:string,options:AnalysisOptions={}){
 ...
 const issueMonitorMatches=await linkOnlineArticleToIssueMonitors(pool,{articleId,publishedAt:article.published_at,title:article.title,content:`${article.summary||''} ${article.content||''}`,taxonomyId:routing?.taxonomyId??null});
