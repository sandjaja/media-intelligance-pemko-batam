...
export async function registerArticleManualClassificationRoutes(app:FastifyInstance,pool:Pool,jwtSecret:string){
 ...
 async function isVerifiedLocked(articleId:number){const r=await pool.query(`SELECT 1 FROM audit_logs WHERE action='ARTICLE_CLASSIFICATION_VERIFIED' AND metadata->>'articleId'=$1 ORDER BY created_at DESC LIMIT 1`,[String(articleId)]);return !!r.rowCount;}
 ...
 async function correctKeyword(articleId:number,organizationId:number,keywordIds:number[],reason:string,actor:AuthorizationContext,article:any){
  ...
  try{
   ...
   for(const keywordId of ids)await client.query(`INSERT INTO article_manual_keywords(article_id,keyword_id,selected_by,reason,active,created_at,updated_at) VALUES($1,$2,$3,$4,true,NOW(),NOW()) ON CONFLICT(article_id,keyword_id) DO UPDATE SET selected_by=EXCLUDED.selected_by,reason=EXCLUDED.reason,active=true,updated_at=NOW()`,[articleId,keywordId,actor.id,reason||null]);
 ...
 app.post('/api/admin/articles/:id/classification-verification',{preHandler:manager},async(request,reply)=>{
  ...
  if(p.data.action==='REOPEN'){
   ...
   await audit(pool,actor.id,'ARTICLE_CLASSIFICATION_REOPENED',{organizationId,articleId:String(articleId),title:article.title,before,reason:p.data.reason});
   return{ok:true,data:{articleId:String(articleId),action:'REOPEN',verificationStatus:'REOPENED',classification:article.news_classification,source:article.news_classification_source}};
  }
