...
function chooseOriginal(members:WorkingMember[]){return members.slice().sort((a,b)=>dateMs(a.mention)-dateMs(b.mention)||Number(a.mention.id)-Number(b.mention.id))[0].mention}
...
export async function rebuildOwnedContentClusters(pool:Pool){
  const{rows}=await pool.query<Mention>(`SELECT id,owned_account_id,title,content,published_at,captured_at FROM social_mentions WHERE source_kind='owned' ORDER BY published_at ASC NULLS LAST,captured_at ASC,id ASC LIMIT 1000`);
  ...
  return{mentions:rows.length,clusters:clusters.length,republished:clusters.filter(c=>c.members.length>1).length,maxChannels:clusters.reduce((m,c)=>Math.max(m,new Set(c.members.map(x=>x.mention.owned_account_id).filter(Boolean)).size),0),engine:'owned-content-rule-v1.1'};
}
