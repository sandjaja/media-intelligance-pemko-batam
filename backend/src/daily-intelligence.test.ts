...
function mockPool(rows: any[]) {
  const calls: any[] = [];
  return { calls, async query(sql: string, params?: unknown[]) { calls.push({ sql, params }); if (sql.startsWith('SELECT a.id')) return { rows }; return { rows: [] }; } } as any;
}
...
test('daily intelligence ranks high-risk negative issue and persists payload', async () => {
  ...
  assert.equal(result.date, '2026-09-05'); assert.equal(result.metrics.totalArticles, 2); assert.equal(result.metrics.negativeCount, 1); assert.equal(result.status, 'ESCALATING'); assert.equal(result.responseWindow, '1–6 JAM'); assert.equal(result.dailyHighlight?.id, '1'); assert.equal(result.topNegative[0]?.id, '1'); assert.equal(result.topMedia?.name, 'Media A'); assert.ok(pool.calls.some((c: any) => c.sql.includes('INSERT INTO daily_intelligence_runs')));
});

test('empty day returns normal monitoring state', async () => {
  const pool = mockPool([]); const result = await generateDailyIntelligence(pool, '2026-09-05'); assert.equal(result.status, 'NORMAL'); assert.equal(result.metrics.totalArticles, 0); assert.equal(result.dailyHighlight, null); assert.match(result.executiveBrief, /Belum ada pemberitaan/);
});
