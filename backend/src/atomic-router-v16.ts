...
export async function routeArticleV16(pool:Pool,articleId:string){
 ...
 const headlineTaxonomies=await getV16HeadlineTaxonomies(pool,String(article.title||''));
 ...
 return{articleId:String(articleId),classificationSource:evidence.matchType==='MANUAL'?'MANUAL_KEYWORD':'AUTO_V16',routingStatus:'ROUTED',headlineTaxonomyIds:headlineTaxonomies.map(t=>t.id),headlineTaxonomyNames:headlineTaxonomies.map(t=>t.name),opdId:evidence.opdId,supportingOpdIds:evidence.supportingOpdIds,districtId:null,uptdId:primaryUnit?.id??null,uptdName:primaryUnit?.name??null,uptdMatches:matches.length,taxonomyId:evidence.taxonomyId,taxonomyName:evidence.taxonomyName,taxonomyScore:evidence.score,issueId:null,issueMatchScore:0,issueAssignmentSource:null,keywordMatches:1,matchedKeywords:[evidence.keyword]};
}
