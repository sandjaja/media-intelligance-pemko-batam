...
export async function registerPrintAnalysisRecoveryRoutes(app:FastifyInstance,pool:Pool,jwtSecret:string){
  ...
  app.post('/api/print/articles/:id/mark-analyzed-v2',{preHandler:auth},async(request,reply)=>{
    ...
    const [official,operator,opds,districts,villages,taxonomyMappings]=await Promise.all([
    ...
    const classified=classifyIssueWithTaxonomy(title,summary,body,taxonomyMappings as any[],operatorMatches);
    const issueCategory=classified.issueCategory;
    const categoryEvidence=classified.categoryEvidence;
    ...
    const signals=[current.is_headline?'Headline/front-page indicator':null,`Kategori isu: ${issueCategory} (score ${categoryEvidence.winnerScore}; ${categoryEvidence.source})`,officialMatches.length?`Keyword resmi terdeteksi: ${(officialMatches as any[]).map(k=>k.keyword).join(', ')}`:null,operatorMatches.length?`Keyword operator valid terkonfirmasi di teks: ${operatorMatches.map(k=>k.keyword).join(', ')}`:null,operatorNoise.length?`Noise keyword diabaikan: ${operatorNoise.slice(0,8).map(k=>k.keyword).join(', ')}`:null,detectedOpdObj?`OPD terdeteksi: ${detectedOpdObj.name} (${opdMatch})`:null,detectedDistrict?`Kecamatan terdeteksi: ${detectedDistrict.name} (${districtMatch})`:null,negative.length?`Sinyal negatif: ${negative.join(', ')}`:null,high.length?`Sinyal risiko: ${high.join(', ')}`:null,critical.length?`Sinyal kritis: ${critical.join(', ')}`:null].filter(Boolean) as string[];
    const analysis={engine:'phase2e-rule-v2.6-keyword-taxonomy',generatedAt:new Date().toISOString(),sentiment,issueCategory,categoryEvidence,riskScore:risk,riskLevel,importanceScore:importance,confidence,officialKeywordMatches:officialMatches,operatorKeywordMatches:operatorMatches,operatorKeywordsNotFound:operatorNotFound,operatorKeywordNoise:operatorNoise,entityValidation,signals:signals.length?signals:['Tidak ada sinyal risiko eksplisit yang kuat pada teks terverifikasi.'],issueLinkage:{engine:'deferred',candidateCount:0,linkedIssueId:null,candidates:[],degraded:true,note:'Issue linkage sementara dipisahkan dari proses analisis inti.'},note:'Phase 2E stable path v2.6: taxonomy database menjadi sumber utama kategori; rule lokal hanya fallback. Keyword OPD dan kategori tetap dua dimensi berbeda dan metadata manusia tetap authoritative.'};
