...
/** Master Keyword is the hard semantic gate; lexical + parent context + time split conversations within that keyword. */
export function clusterSocialConversations(rows:SocialConversationItem[]):SocialConversationCluster[]{
  const eligible=rows.filter(row=>{const r=routing(row);return r.routingStatus==='ROUTED'&&clean(r.taxonomyId)&&clean(r.taxonomyName)&&clean(r.keywordId)}).sort((a,b)=>timeOf(a)-timeOf(b));
  ...
  for(const row of eligible){
    ...
    for(const g of groups){
      ...
      const sameKeyword=Boolean(keywordId&&g.keywordId===keywordId);
      if(!sameKeyword)continue;
      const score=Math.max(...g.mentions.map(m=>similarity(rowTokens,tokens(m))));
      if(score>=0.18){const weighted=score+0.15;if(weighted>bestScore){best=g;bestScore=weighted;}}
    }
