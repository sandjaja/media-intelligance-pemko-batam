...
export async function registerOnlineStoryClusterRoutes(app:FastifyInstance,pool:Pool,jwtSecret:string){
  ...
  app.get('/api/online/articles/:articleId/issue-linkage',{preHandler:auth},async(request,reply)=>{
    ...
    const recommended=issues.map((i:any)=>{const iw=words(i.title+' '+(i.description||''));let overlap=0;for(const w of aw)if(iw.has(w))overlap++;const semantic=Math.min(65,overlap*13),opd=article.opd_id&&i.opd_ids.map(Number).includes(Number(article.opd_id))?25:0;return{...i,score:Math.min(100,semantic+opd),reasons:[semantic?`Kesamaan topik/keyword: ${overlap} token`:null,opd?'Primary OPD sama':null].filter(Boolean)}}).filter((i:any)=>i.score>=30).sort((a:any,b:any)=>b.score-a.score).slice(0,3);
