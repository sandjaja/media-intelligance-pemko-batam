...
async function ensureIssueLinkageSchema(db:Pool|PoolClient){
  ...
  await db.query(`CREATE INDEX IF NOT EXISTS idx_issue_print_articles_print ON issue_print_articles(print_article_id, linkage_status, relevance_score DESC)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_issue_print_articles_issue ON issue_print_articles(issue_id, linkage_status, relevance_score DESC)`);
}
