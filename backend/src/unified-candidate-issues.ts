...
const ENGINE='unified-candidate-issue-v1.3-owned-merge-fix';
...
function inferCategory(title:string,summary:string){
  ...
  if(/\b(apbd|rapbd|pad|anggaran|defisit|pendapatan daerah|belanja daerah|keuangan daerah)\b/i.test(text))return 'Pemerintahan / Keuangan Daerah';
  if(/\b(investasi|ekonomi|industri|umkm|inflasi|pengangguran|pariwisata|tourism)\b/i.test(text))return 'Ekonomi & Investasi';
  if(/\b(banjir|genangan|drainase|jalan|jembatan|transportasi|macet|pelabuhan)\b/i.test(text))return 'Infrastruktur & Transportasi';
  if(/\b(sampah|limbah|lingkungan|pencemaran|kebersihan)\b/i.test(text))return 'Lingkungan';
  if(/\b(pelayanan|layanan publik|administrasi|perizinan|kependudukan)\b/i.test(text))return 'Pelayanan Publik';
  if(/\b(olahraga|atlet|popda|porprov|sport)\b/i.test(text))return 'Olahraga & Kepemudaan';
  return null;
}
...
async function resolveOrganizationId(db:Pool|PoolClient,ctx:AuthorizationContext){
  ...
  const r=await db.query('SELECT id FROM organizations ORDER BY id LIMIT 2');return r.rowCount===1?Number(r.rows[0].id):0;
}
...
function looksLikeNoise(title:string,summary:string){
  const raw=String(title||'').trim(),n=norm(raw),ts=tokens(`${title} ${summary}`);
  if(raw.length<12||ts.length<2)return true;
  ...
  const letters=(n.match(/[a-z]/g)||[]).length,digits=(n.match(/[0-9]/g)||[]).length;
...
function quality(title:string,summary:string,importance:number,risk:number,category:string|null){
  ...
  if(title.trim().length>=20)score+=10;if(ts.length>=3)score+=15;else if(ts.length>=2)score+=8;
  if(category)score+=10;if(importance>=40)score+=15;else if(importance>=30)score+=8;
...
function issueSimilarity(e:any,i:any){
  const a=new Set(tokens(`${e.title} ${e.summary||''} ${e.category||''}`)),b=new Set(tokens(`${i.title} ${i.description||''}`));
...
async function detect(db:Pool|PoolClient,organizationId:number){
  ...
  const evidence=[...print,...online,...owned].map((r:any)=>{
    const title=String(r.title||'').trim(),summary=String(r.summary||r.content||'').slice(0,1500);
    return {sourceType:r.source_type,id:Number(r.id),title,summary,sentiment:r.sentiment||null,riskScore:Number(r.risk_score||0),importanceScore:Number(r.importance_score||0),opdId:r.opd_id==null?null:Number(r.opd_id),opdName:r.opd_name||null,districtId:r.district_id==null?null:Number(r.district_id),category:r.category||inferCategory(title,summary),occurredAt:r.occurred_at||null,accountName:r.account_name||null,officialOrigin:r.source_type==='owned'};
  });
...
async function linkEvidence(client:PoolClient,issueId:number,c:any,ctx:AuthorizationContext,reason:string){
  for(const e of c.evidence){
    ...
    if(e.sourceType==='owned')await client.query(`INSERT INTO social_mention_issues(mention_id,issue_id,relevance_score,linkage_source) VALUES($1,$2,$3,'manual') ON CONFLICT(mention_id,issue_id) DO UPDATE SET relevance_score=EXCLUDED.relevance_score,linkage_source='manual'`,[e.id,issueId,c.score]);
  }
}
...
export async function registerUnifiedCandidateIssueRoutes(app:FastifyInstance,pool:Pool,jwtSecret:string){
  ...
  app.post('/api/intelligence/unified-candidate-issues/decision',{preHandler:auth},async(request,reply)=>{
    ...
    const client=await pool.connect();try{
      await client.query('BEGIN');const organizationId=await resolveOrganizationId(client,ctx);if(!organizationId){await client.query('ROLLBACK');return reply.code(409).send({error:'ORGANIZATION_UNRESOLVED'})}
      const c=(await detect(client,organizationId)).find(x=>x.candidateKey===p.data.candidateKey);if(!c){await client.query('ROLLBACK');return reply.code(409).send({error:'CANDIDATE_NOT_AVAILABLE'})}
      ...
      if(p.data.decision==='merge'){
        ...
        const slug=c.candidateKey.replace(/[^a-z0-9]+/gi,'-').toLowerCase(),issueKey=`uci-${slug}`;
