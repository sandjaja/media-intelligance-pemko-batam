...
function fakePool(){
 return{
  async query(sql:string,params?:unknown[]){
   ...
   if(sql.includes('FROM opd WHERE organization_id='))return{rows:[{id:'20',name:'Dinas Pelayanan',code:'DP'},{id:'30',name:'Dinas Perhubungan',code:'DISHUB'}]};
...
test('Social V17 returns PENDUKUNG when taxonomy is visible but Master Keyword and OPD are absent',async()=>{
 const result=await analyzeSocialRoutingV16(fakePool(),{
...
test('Social V17 returns AMBIGU when OPD is visible but Master Keyword is absent',async()=>{
 const result=await analyzeSocialRoutingV16(fakePool(),{
  title:'Dishub meninjau lokasi',
  content:'Warga meminta tindak lanjut.'
 });
 assert.equal(result.routingStatus,'AMBIGUOUS');
 assert.equal(result.newsClassification,'AMBIGU');
 assert.equal(result.keywordId,null);
 assert.equal(result.primaryOpdId,'30');
 assert.equal(result.primaryOpdName,'Dinas Perhubungan');
 assert.equal(result.needsVerification,true);
});
