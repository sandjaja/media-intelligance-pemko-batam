...
export async function registerOwnedSocialRoutes(app: FastifyInstance, pool: Pool, jwtSecret: string) {
  const auth = async (request: FastifyRequest, reply: any) => {
    ...
    try {
      ...
      const { rows } = await pool.query(`SELECT u.id,u.active, COALESCE(array_agg(DISTINCT r.code) FILTER (WHERE r.code IS NOT NULL), ARRAY[]::text[]) roles, COALESCE(array_agg(DISTINCT p.code) FILTER (WHERE p.code IS NOT NULL), ARRAY[]::text[]) permissions FROM users u LEFT JOIN user_roles ur ON ur.user_id=u.id LEFT JOIN roles r ON r.id=ur.role_id LEFT JOIN role_permissions rp ON rp.role_id=r.id LEFT JOIN permissions p ON p.id=rp.permission_id WHERE u.id=$1 GROUP BY u.id,u.active`, [d.sub]);
      const user = rows[0];
      const roles = Array.isArray(user?.roles) ? user.roles.map(String) : [];
      const permissions = Array.isArray(user?.permissions) ? user.permissions.map(String) : [];
      if (user?.active !== true || (!roles.includes('super_admin') && !roles.includes('humas') && !permissions.includes('platform.admin') && !permissions.includes('sources.manage'))) return reply.code(403).send({ error: 'FORBIDDEN' });
  ...
  app.delete('/api/admin/owned-social-accounts/:id', { preHandler: auth }, async (request, reply) => {
    const id = idParam.safeParse(request.params); if (!id.success) return reply.code(400).send({ error: 'INVALID_OWNED_SOCIAL_ACCOUNT' });
    const { rows } = await pool.query(`UPDATE owned_social_accounts SET active=false,updated_at=now() WHERE id=$1 RETURNING id,active`, [id.data.id]); if (!rows[0]) return reply.code(404).send({ error: 'OWNED_SOCIAL_ACCOUNT_NOT_FOUND' });
    await audit((request as any).ownedSocialAdminUserId,'OWNED_SOCIAL_ACCOUNT_DEACTIVATED',{ accountId: id.data.id }); return { data: rows[0] };
  });
}