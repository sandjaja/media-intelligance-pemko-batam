...
export async function registerPhase2gMediaSummaryRoutes(app:FastifyInstance,pool:Pool,jwtSecret:string){
 ...
 app.get('/api/intelligence/media-summary',{preHandler:auth},async(request,reply)=>{
  ...
  const print=await pool.query(`SELECT COUNT(*)::int total,COUNT(*) FILTER(WHERE pa.sentiment='positive')::int positive,COUNT(*) FILTER(WHERE pa.sentiment='neutral')::int neutral,COUNT(*) FILTER(WHERE pa.sentiment='negative')::int negative,COUNT(*) FILTER(WHERE pa.risk_score<35)::int low,COUNT(*) FILTER(WHERE pa.risk_score>=35 AND pa.risk_score<60)::int medium,COUNT(*) FILTER(WHERE pa.risk_score>=60 AND pa.risk_score<80)::int high,COUNT(*) FILTER(WHERE pa.risk_score>=80)::int critical,COALESCE(ROUND(AVG(pa.risk_score)),0)::int avg_risk,COALESCE(ROUND(AVG(pa.importance_score)),0)::int avg_importance FROM print_articles pa WHERE ${pw.join(' AND ')}`,params);
  const issues=await pool.query(`SELECT i.id,i.title,i.status,i.risk_level,COUNT(DISTINCT ipa.print_article_id)::int evidence_count,COALESCE(ROUND(AVG(pa.risk_score)),0)::int avg_risk,MAX(pa.risk_score)::int max_risk FROM issue_print_articles ipa JOIN print_articles pa ON pa.id=ipa.print_article_id JOIN issues i ON i.id=ipa.issue_id WHERE ipa.linkage_status='linked' AND ${pw.join(' AND ')} GROUP BY i.id,i.title,i.status,i.risk_level ORDER BY max_risk DESC,evidence_count DESC LIMIT 10`,params);
  ...
  const top=await pool.query(`SELECT pa.id,pa.title,pa.sentiment,pa.risk_score,CASE WHEN pa.risk_score>=80 THEN 'critical' WHEN pa.risk_score>=60 THEN 'high' WHEN pa.risk_score>=35 THEN 'medium' ELSE 'low' END risk_level,pa.importance_score,pa.created_at,o.name opd_name,d.name district_name FROM print_articles pa LEFT JOIN opd o ON o.id=pa.opd_id LEFT JOIN districts d ON d.id=pa.district_id WHERE ${pw.join(' AND ')} ORDER BY pa.risk_score DESC,pa.importance_score DESC,pa.created_at DESC LIMIT 10`,params);
  return{data:{engine:'phase2g-media-summary-v1.1',generatedAt:new Date().toISOString(),filters:{from:q.data.from??null,to:q.data.to??null,opdId:opdId??null,districtId:q.data.districtId??null},print:print.rows[0],topIssues:issues.rows,activeAlerts:alerts.rows,topPrintEvidence:top.rows}};
 });
}
