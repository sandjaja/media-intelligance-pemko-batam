...
export async function ingestSocialBatch(pool:Pool,candidates:SocialCandidate[],collector='social-batch'){const results:Record<string,unknown>[]=[];for(const candidate of candidates){try{const result=await ingestSocialCandidate(pool,candidate,collector);results.push({ok:true,...result})}catch(error){results.push({ok:false,platform:candidate.platform,externalId:candidate.externalId??null,error:error instanceof Error?error.message:String(error)})}}return{received:candidates.length,succeeded:results.filter(r=>r.ok===true&&r.skipped!==true).length,failed:results.filter(r=>r.ok!==true).length,results}}
