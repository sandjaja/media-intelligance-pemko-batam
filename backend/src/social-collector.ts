...
export async function ingestSocialCandidate(pool:Pool,candidate:SocialCandidate,defaultCollector='social-batch'){
 ...
 const monitorMatches=candidate.sourceKind==='owned'&&curationStatus==='candidate'?[]:await linkSocialMentionToIssueMonitors(pool,{mentionId:Number(mention.id),sourceKind:candidate.sourceKind,publishedAt,title:candidate.title,content:routingContent,taxonomyId:routing.taxonomyId});
