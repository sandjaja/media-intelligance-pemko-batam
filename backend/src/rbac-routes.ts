...
export async function registerRbacRoutes(app: FastifyInstance, pool: Pool, jwtSecret: string) {
  ...
  app.get('/api/admin/rbac/users', { preHandler: [authz, requirePermission('users.manage')] }, async () => {
    const { rows } = await pool.query(`SELECT u.id,u.email,u.role legacy_role,u.active,u.opd_id,o.name opd_name,u.created_at, COALESCE(array_agg(DISTINCT r.code) FILTER (WHERE r.code IS NOT NULL), ARRAY[]::text[]) roles FROM users u LEFT JOIN opd o ON o.id=u.opd_id LEFT JOIN user_roles ur ON ur.user_id=u.id LEFT JOIN roles r ON r.id=ur.role_id GROUP BY u.id,u.email,u.role,u.active,u.opd_id,o.name,u.created_at ORDER BY u.email`);
    return { data: rows };
  });

  const createInput = z.object({ email: z.string().email().max(200), password: z.string().min(8).max(200), role: roleEnum, opdId: z.coerce.number().int().positive().nullable().optional(), active: z.boolean().default(true) });
  app.post('/api/admin/rbac/users', { preHandler: [authz, requirePermission('users.manage')] }, async (request, reply) => {
    ...
    const role = parsed.data.role as NormalizedRole, opdId = parsed.data.opdId ?? null;
    ...
    try { await client.query('BEGIN'); const { rows } = await client.query(`INSERT INTO users(email,password_hash,role,active,opd_id) VALUES($1,$2,$3,$4,$5) RETURNING id,email,role,active,opd_id,created_at`, [parsed.data.email.toLowerCase(), passwordHash, legacyRoleFor(role), parsed.data.active, opdId]); await client.query(`INSERT INTO user_roles(user_id,role_id,opd_id) SELECT $1,r.id,$2 FROM roles r WHERE r.code=$3 ON CONFLICT DO NOTHING`, [rows[0].id, opdId, role]); await client.query(`INSERT INTO audit_logs(user_id,action,metadata) VALUES($1,'RBAC_USER_CREATED',$2)`, [request.authz!.id, { userId: rows[0].id, email: rows[0].email, normalizedRole: role, opdId }]); await client.query('COMMIT'); return reply.code(201).send({ data: { ...rows[0], roles: [role] } }); }
  ...
  const updateInput = z.object({ email: z.string().email().max(200).optional(), password: z.string().min(8).max(200).optional(), role: roleEnum.optional(), opdId: z.coerce.number().int().positive().nullable().optional(), active: z.boolean().optional() });
  app.patch('/api/admin/rbac/users/:id', { preHandler: [authz, requirePermission('users.manage')] }, async (request, reply) => {
    ...
    const role=(parsed.data.role??current.normalized_role) as NormalizedRole, opdId=parsed.data.opdId===undefined?(current.opd_id??null):parsed.data.opdId;
    if(roleRequiresOpd(role)&&!opdId)return reply.code(400).send({error:'OPD_REQUIRED_FOR_ROLE'}); if(!roleRequiresOpd(role)&&opdId)return reply.code(400).send({error:'GLOBAL_ROLE_CANNOT_HAVE_OPD_SCOPE'});
