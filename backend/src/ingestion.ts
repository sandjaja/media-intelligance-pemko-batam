...
export type FeedSource = { id: string; name: string; url: string; tier?: number; enabled?: boolean };
export type IngestedArticle = { sourceId: string; title: string; url: string; publishedAt: Date; excerpt?: string; content?: string };

const parser = new XMLParser({ ignoreAttributes: false, attributeNamePrefix: '@_' });

function asArray<T>(value: T | T[] | undefined): T[] { return value == null ? [] : Array.isArray(value) ? value : [value]; }
function firstString(...values: unknown[]): string | undefined { return values.find(v => typeof v === 'string' && v.trim()) as string | undefined; }
function parseDate(value?: string): Date { const d = value ? new Date(value) : new Date(); return Number.isNaN(d.getTime()) ? new Date() : d; }
...
export async function fetchFeed(source: FeedSource): Promise<IngestedArticle[]> {
  ...
  return items.map((item: any) => {
    ...
    return {
      ...
      title: title.trim(),
      url: url.trim(),
      publishedAt: parseDate(firstString(item.pubDate, item.published, item.updated, item['dc:date'])),
      excerpt: firstString(item.description, item.summary, item['content:encoded'])?.slice(0, 5000)
    };
...
export async function ingestSource(pool: Pool, source: FeedSource): Promise<{ fetched: number; inserted: number }> {
  ...
  for (const article of articles) {
    ...
    const result = await pool.query(
      `INSERT INTO articles (source_id, title, url, published_at, content, sentiment, importance_score)
       VALUES ($1,$2,$3,$4,$5,'NEUTRAL',0)
       ON CONFLICT (url) DO NOTHING RETURNING id`,
      [article.sourceId, article.title, article.url, article.publishedAt, article.content ?? article.excerpt ?? null]
    );
    if (result.rowCount) inserted++;
    // Keep the fingerprint available for future stronger cross-source deduplication.
    await pool.query(`INSERT INTO audit_logs (action, entity, entity_id, metadata) VALUES ('INGEST_ARTICLE','article',$1,$2)`, [result.rows[0]?.id ?? null, { fingerprint: fp, sourceId: source.id }]).catch(() => undefined);
  }
...
export async function ingestEnabledSources(pool: Pool): Promise<Record<string, unknown>[]> {
  const { rows } = await pool.query(`SELECT id, name, feed_url AS url, tier, enabled FROM media_sources WHERE enabled = true AND feed_url IS NOT NULL`);
