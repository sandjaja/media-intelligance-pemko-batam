...
const cluster=(title:string,content='')=>({id:'1',canonical_title:title,representative_title:title,representative_content:content,representative_source:'Media Center Batam',member_count:1,channel_count:1,first_published_at:null,last_published_at:null});
...
test('matches APBD 2027 coverage to the official RAPBD 2027 narrative',()=>{
  const m=evaluateCommunicationGapMatch(
...
test('does not merge PAD narrative with APBD deficit merely because both concern finance',()=>{
  const m=evaluateCommunicationGapMatch(
