...
import { checkIssueEvidenceEligibility } from './issue-evidence-eligibility.js';
...
async function analyzeLockedSocialMention(pool:Pool,mentionId:number){const mention=(await pool.query(`SELECT id,title,content,metadata,opd_id FROM social_mentions WHERE id=$1`,[mentionId])).rows[0];if(!mention)return false;const routing=mention.metadata?.v16Routing||{};if(routing.newsClassification!=='UTAMA'||routing.routingStatus!=='ROUTED'||!mention.opd_id)return false;const query=parseKeywordQuery(routing.keyword?[routing.keyword].join(' | '):'');const analysis=analyzeCoreArticle({id:mention.id,title:String(mention.title||mention.content||'').slice(0,300),summary:String(mention.content||'').slice(0,900),content:mention.content??null,sourceName:'social',sourceTier:null,mediaKind:'social',opdId:Number(mention.opd_id),publishedAt:null},query,1);const risk=calculateRisk({importance:analysis.importanceScore,impact:analysis.impactScore,velocity:analysis.velocityScore,sentiment:analysis.sentiment,sentimentScore:analysis.sentimentScore});const metadata={...(mention.metadata||{}),intelligence:{...analysis,riskLevel:risk.level,riskReasons:risk.reasons,riskStatus:'FINAL'}};await pool.query(`UPDATE social_mentions SET metadata=$2::jsonb,sentiment=$3,sentiment_score=$4,importance_score=$5,influence_score=$6,risk_score=$7,risk_level=$8,updated_at=NOW() WHERE id=$1`,[mentionId,JSON.stringify(metadata),analysis.sentiment,analysis.sentimentScore,analysis.importanceScore,analysis.impactScore,risk.score,risk.level]);return true;}
...
export async function registerSocialIntelligenceRoutes(app: FastifyInstance, pool: Pool, jwtSecret: string) {
  ...
  app.post('/api/admin/social/:id/scope-approve',{preHandler:manager},async(request,reply)=>{
    const mentionId=Number((request.params as any).id);
    if(!Number.isInteger(mentionId)||mentionId<=0)return reply.code(400).send({error:'INVALID_MENTION_ID'});
    const actor=request.socialAuth!,organizationId=await resolveOrganizationId(actor);
    if(!organizationId)return reply.code(409).send({error:'ORGANIZATION_UNRESOLVED'});
    const mention=(await pool.query(`SELECT id,title,content,source_kind,metadata,opd_id FROM social_mentions WHERE id=$1`,[mentionId])).rows[0];
    ...
    const routing=await analyzeSocialRoutingV16(pool,{title:mention.title,content:mention.content});
    const approvedScope={...(mention.metadata.organizationScope||{}),status:'RELEVANT',approvedBy:actor.id,approvedAt:new Date().toISOString(),approvalSource:'HUMAN'};
    const {intelligence:_oldIntelligence,...metadataWithoutIntelligence}=mention.metadata||{};
    const metadata={...metadataWithoutIntelligence,organizationScope:approvedScope,v16Routing:routing};
    const client=await pool.connect();
    try{
      await client.query('BEGIN');
    ...
    }catch(e){await client.query('ROLLBACK');request.log.error({err:e,mentionId},'social scope approval failed');return reply.code(409).send({error:'SOCIAL_SCOPE_APPROVAL_FAILED'});}finally{client.release();}
  });
  ...
  app.post('/api/admin/social/:id/classification-verification',{preHandler:manager},async(request,reply)=>{
    ...
    if(p.data.action==='APPROVE'){
      ...
      await pool.query(`UPDATE social_mentions SET metadata=jsonb_set(jsonb_set(COALESCE(metadata,'{}'::jsonb),'{socialVerification}',$2::jsonb,true),'{intelligence,riskStatus}',to_jsonb('FINAL'::text),true),updated_at=NOW() WHERE id=$1`,[mentionId,JSON.stringify(verification)]);
      ...
      await analyzeLockedSocialMention(pool,mentionId);
      ...
      let unifiedIssueTriggered=false;try{await detectUnifiedIssueCandidates(pool,organizationId,{sourceType:'social',evidenceId:mentionId});unifiedIssueTriggered=true;}catch(e){request.log.warn({err:e,mentionId},'Unified Issue social resolution skipped');}
      return{ok:true,data:{mentionId:String(mentionId),verificationStatus:'LOCKED',issueMonitorMatches,unifiedIssueTriggered}};
    }
  ...
  app.post('/api/admin/social/classification-verification/bulk',{preHandler:manager},async(request,reply)=>{
    ...
    for(const mentionId of ids){try{
      ...
      await pool.query(`UPDATE social_mentions SET metadata=jsonb_set(jsonb_set(COALESCE(metadata,'{}'::jsonb),'{socialVerification}',$2::jsonb,true),'{intelligence,riskStatus}',to_jsonb('FINAL'::text),true),updated_at=NOW() WHERE id=$1`,[mentionId,JSON.stringify(verification)]);
      await pool.query(`INSERT INTO audit_logs(user_id,action,metadata) VALUES($1,'SOCIAL_CLASSIFICATION_VERIFIED',$2::jsonb)`,[actor.id,JSON.stringify({mentionId:String(mentionId),classification:'UTAMA',opdId:mention.opd_id,bulk:true})]);await analyzeLockedSocialMention(pool,mentionId);try{const evidence=(await pool.query(`SELECT published_at,title,content,metadata->'v16Routing'->>'taxonomyId' taxonomy_id FROM social_mentions WHERE id=$1`,[mentionId])).rows[0];if(evidence)await linkEligibleSocial(pool,{mentionId,sourceKind:'external',publishedAt:evidence.published_at,title:evidence.title,content:evidence.content,taxonomyId:evidence.taxonomy_id?Number(evidence.taxonomy_id):null});}catch(e){request.log.warn({err:e,mentionId},'Issue Monitor bulk social linkage skipped');}try{await detectUnifiedIssueCandidates(pool,organizationId,{sourceType:'social',evidenceId:mentionId});}catch(e){request.log.warn({err:e,mentionId},'Unified Issue bulk social resolution skipped');}locked++;
  ...
  app.post('/api/social/mentions/:id/issues', { preHandler: [auth, requireWrite] }, async (request, reply) => {
    ...
    const client=await pool.connect();try{await client.query('BEGIN');const mention=(await client.query(`SELECT source_kind FROM social_mentions WHERE id=$1`,[id.data])).rows[0];if(!mention){await client.query('ROLLBACK');return reply.code(404).send({error:'MENTION_NOT_FOUND'});}if(mention.source_kind==='external'){const gate=await checkIssueEvidenceEligibility(client,'SOCIAL',id.data);if(!gate.eligible){await client.query('ROLLBACK');return reply.code(409).send({error:'SOCIAL_ISSUE_EVIDENCE_INELIGIBLE',reason:gate.reason});}}const previousLinked=body.data.linkageSource==='manual'?(await client.query(`SELECT issue_id FROM social_mention_issues WHERE mention_id=$1 AND issue_id<>$2 AND linkage_source='manual'`,[id.data,body.data.issueId])).rows.map((x:any)=>Number(x.issue_id)):[];if(body.data.linkageSource==='manual')await client.query(`DELETE FROM social_mention_issues WHERE mention_id=$1 AND issue_id<>$2 AND linkage_source='manual'`,[id.data,body.data.issueId]);const {rows}=await client.query(`INSERT INTO social_mention_issues(mention_id,issue_id,relevance_score,linkage_source) VALUES($1,$2,$3,$4) ON CONFLICT(mention_id,issue_id) DO UPDATE SET relevance_score=EXCLUDED.relevance_score,linkage_source=EXCLUDED.linkage_source RETURNING *`,[id.data,body.data.issueId,body.data.relevanceScore,body.data.linkageSource]);if(mention.source_kind==='external')for(const issueId of [...new Set([body.data.issueId,...previousLinked])])await recalculateIssueRisk(client,issueId);await client.query('COMMIT');return{data:rows[0]};}catch(e){await client.query('ROLLBACK');throw e;}finally{client.release();}
  });
  ...
  app.get('/api/social/conversation-clusters',{preHandler:auth},async(request,reply)=>{
    ...
    // Keep cluster cards on the same filtered evidence scope used by cluster
    // Issue Linkage: count, platform count and members all follow the active
    // dashboard filters.
  ...
  app.get('/api/social/conversation-clusters/:id/issue-linkage',{preHandler:auth},async(request,reply)=>{const id=z.coerce.number().int().positive().safeParse((request.params as any).id);if(!id.success)return reply.code(400).send({error:'INVALID_ID'});const organizationId=await resolveOrganizationId(request.socialAuth!);if(!organizationId)return reply.code(409).send({error:'ORGANIZATION_UNRESOLVED'});const cluster=(await pool.query(`SELECT id,canonical_title FROM social_conversation_clusters WHERE id=$1 AND organization_id=$2 AND status='ACTIVE'`,[id.data,organizationId])).rows[0];if(!cluster)return reply.code(404).send({error:'CLUSTER_NOT_FOUND'});const members=(await pool.query(`SELECT sm.id,sm.metadata FROM social_conversation_cluster_members cm JOIN social_mentions sm ON sm.id=cm.mention_id WHERE cm.cluster_id=$1 AND sm.source_kind='external' AND COALESCE(sm.metadata->'organizationScope'->>'status','RELEVANT')='RELEVANT' AND COALESCE(sm.published_at,sm.captured_at)>=NOW()-($2::int*INTERVAL '1 day')`,[id.data,Number((request.query as any)?.days)||7])).rows;const eligibility=await Promise.all(members.map(async(m:any)=>({member:m,result:await checkIssueEvidenceEligibility(pool,'SOCIAL',Number(m.id))})));const eligible=eligibility.filter((x:any)=>x.result.eligible).map((x:any)=>x.member);let candidate:any=null;for(const m of eligible){const cs=await detectUnifiedIssueCandidates(pool,organizationId,{sourceType:'social',evidenceId:Number(m.id)});candidate=cs.find((c:any)=>Array.isArray(c.evidence)&&c.evidence.some((e:any)=>e.sourceType==='social'&&eligible.some((x:any)=>Number(x.id)===Number(e.id))))||candidate;}const ids=eligible.map((m:any)=>Number(m.id));const links=ids.length?(await pool.query(`SELECT smi.issue_id,i.title,COUNT(*)::int count FROM social_mention_issues smi JOIN issues i ON i.id=smi.issue_id WHERE smi.mention_id=ANY($1::bigint[]) GROUP BY smi.issue_id,i.title ORDER BY COUNT(*) DESC`,[ids])).rows:[];const issues=(await pool.query(`SELECT id AS "issueId",title,status FROM issues WHERE organization_id=$1 AND lower(status) IN ('active','watch') ORDER BY updated_at DESC,id DESC`,[organizationId])).rows;return{data:{clusterId:id.data,title:cluster.canonical_title,total:members.length,eligible:eligible.length,ineligible:members.length-eligible.length,currentIssue:links[0]||null,newCandidate:candidate?{candidateKey:candidate.candidateKey,suggestedTitle:candidate.suggestedTitle,reason:`Kandidat lintas-media dengan ${candidate.evidenceCount||candidate.evidence?.length||0} evidence.`}:null,availableIssues:issues,eligibleIds:ids}};});
