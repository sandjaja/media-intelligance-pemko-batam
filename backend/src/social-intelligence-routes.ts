...
export async function registerSocialIntelligenceRoutes(app: FastifyInstance, pool: Pool, jwtSecret: string) {
  ...
  const refreshConversationCluster=async(client:any,clusterId:number|string)=>{
    ...
    if(!stats?.member_count){await client.query(`UPDATE social_conversation_clusters SET member_count=0,platform_count=0,representative_mention_id=NULL,first_published_at=NULL,last_published_at=NULL,status=CASE WHEN origin_mode='SYSTEM' THEN 'ARCHIVED' ELSE status END,updated_at=NOW() WHERE id=$1`,[clusterId]);return;}
