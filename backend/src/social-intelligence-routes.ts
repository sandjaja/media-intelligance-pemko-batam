...
export async function registerSocialIntelligenceRoutes(app: FastifyInstance, pool: Pool, jwtSecret: string) {
  ...
  app.get('/api/social/conversation-clusters',{preHandler:auth},async(request,reply)=>{
    ...
    // The active filters decide which clusters are visible, but the card count and
    // members represent the cluster's complete current membership. This keeps
    // "N mention" consistent with "Lihat isi & kelola" and cluster Issue Linkage.
    const {rows}=await pool.query(`SELECT c.id,c.canonical_title,c.taxonomy_id,c.keyword_id,c.origin_mode,c.status,MIN(COALESCE(all_sm.published_at,all_sm.captured_at)) first_published_at,MAX(COALESCE(all_sm.published_at,all_sm.captured_at)) last_published_at,COUNT(all_sm.id)::int member_count,COUNT(DISTINCT all_sm.platform)::int platform_count,COALESCE(jsonb_agg(jsonb_build_object('id',all_sm.id,'platform',all_sm.platform,'title',all_sm.title,'content',all_sm.content,'authorName',all_sm.author_name,'authorHandle',all_sm.author_handle,'publishedAt',all_sm.published_at,'capturedAt',all_sm.captured_at,'sentiment',all_sm.sentiment,'riskLevel',all_sm.risk_level,'riskScore',all_sm.risk_score,'assignmentMode',all_cm.assignment_mode) ORDER BY COALESCE(all_sm.published_at,all_sm.captured_at) DESC) FILTER(WHERE all_sm.id IS NOT NULL),'[]'::jsonb) members FROM social_conversation_clusters c JOIN social_conversation_cluster_members all_cm ON all_cm.cluster_id=c.id JOIN social_mentions all_sm ON all_sm.id=all_cm.mention_id AND COALESCE(all_sm.metadata->'organizationScope'->>'status','RELEVANT')='RELEVANT' WHERE c.organization_id=$1 AND c.status='ACTIVE' AND EXISTS (SELECT 1 FROM social_conversation_cluster_members fcm JOIN social_mentions sm ON sm.id=fcm.mention_id WHERE fcm.cluster_id=c.id AND ${memberFilter}) GROUP BY c.id HAVING COUNT(all_sm.id)>0 ORDER BY COUNT(all_sm.id) DESC,MAX(COALESCE(all_sm.published_at,all_sm.captured_at)) DESC`,params);
    return{data:rows};
  });
