(()=>{
  ...
  async function load(){
    ...
    loading=fetch(base()+'/online/story-clusters?days=7&limit=300',{credentials:'include'}).then(async r=>{const b=await r.json().catch(()=>({}));if(!r.ok)throw new Error(b.message||b.error||`HTTP ${r.status}`);return b.data||[];}).then(rows=>{cache=rows;return rows;}).finally(()=>loading=null);
    return loading;
  }
  ...
  async function apply(){
    ...
    let clusters;try{clusters=await load();}catch(e){console.warn('Story cluster UI:',e);return;}
    const byArticle=new Map();for(const c of clusters)for(const m of(Array.isArray(c.publications)?c.publications:[]))byArticle.set(String(m.article_id),c);
