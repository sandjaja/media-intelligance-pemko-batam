(()=>{
  ...
  function memberCard(m,articleEl){if(articleEl){const clone=articleEl.cloneNode(true);clone.style.display='';clone.removeAttribute('data-story-cluster-hidden');clone.removeAttribute('data-story-cluster-representative');clone.removeAttribute('data-story-cluster-id');clone.querySelectorAll('.onlineStoryClusterBox').forEach(x=>x.remove());clone.querySelectorAll('[data-story-cluster-id]').forEach(x=>x.removeAttribute('data-story-cluster-id'));clone.classList.add('rounded-xl','border','border-slate-800','bg-slate-950/60','p-3');return clone.outerHTML;}return `<div class="rounded-lg border border-slate-800 bg-slate-950/60 p-3"><div class="flex flex-wrap items-center justify-between gap-2"><span class="text-[10px] font-bold text-cyan-300">${esc(m.source_name||'-')}</span><span class="text-[10px] text-slate-500">${fmt(m.published_at)}</span></div><a href="${esc(m.url||'#')}" target="_blank" rel="noopener" class="block mt-1 text-xs text-slate-200 hover:text-cyan-300">${esc(m.title||'-')}</a><div class="text-[9px] text-slate-500 mt-1">#${esc(m.article_id)} · ${esc(m.similarity_type||'-')} · similarity ${Number(m.similarity_score||0).toFixed(2)}</div></div>`;}
  function decorate(article,cluster){
    ...
    const articleMap=new Map([...document.querySelectorAll('#online article[data-online-article-id]')].map(a=>[String(a.getAttribute('data-online-article-id')),a]));const clusterTitle=cluster.canonical_title||cluster.story_title||cluster.title||publications[0]?.title||'Story Media Online';box.innerHTML=`<div class="mb-3"><div class="text-[9px] uppercase tracking-widest text-violet-400 font-black">Judul Kluster</div><div class="mt-1 text-sm font-bold text-slate-100">${esc(clusterTitle)}</div></div><div class="flex flex-wrap items-center justify-between gap-2"><div class="flex flex-wrap gap-2 text-[10px]"><span class="px-2 py-1 rounded bg-violet-500/15 text-violet-300 font-bold"><i class="fa-solid fa-layer-group mr-1"></i>${Number(cluster.source_count||0)} Media</span><span class="px-2 py-1 rounded bg-slate-800 text-slate-300">${Number(cluster.member_count||0)} Publikasi</span></div><button type="button" class="onlineStoryToggle text-[10px] font-semibold text-violet-300 hover:text-violet-200">Lihat ${Number(cluster.member_count||0)} Publikasi <i class="fa-solid fa-chevron-down ml-1"></i></button></div><div class="onlineStoryMembers hidden mt-3 space-y-3">${publications.map(m=>memberCard(m,articleMap.get(String(m.article_id)))).join('')}</div>`;
    box.querySelector('.onlineStoryToggle')?.addEventListener('click',()=>{const members=box.querySelector('.onlineStoryMembers'),btn=box.querySelector('.onlineStoryToggle');const open=members?.classList.contains('hidden');members?.classList.toggle('hidden',!open);if(btn)btn.innerHTML=`${open?'Tutup':'Lihat '+Number(cluster.member_count||0)+' Publikasi'} <i class="fa-solid fa-chevron-${open?'up':'down'} ml-1"></i>`;});box.querySelectorAll('.onlineApprove').forEach(b=>b.addEventListener('click',()=>document.querySelector(`#online article[data-online-article-id="${CSS.escape(String(b.dataset.approveId))}"] .onlineApprove`)?.click()));box.querySelectorAll('.onlineClassificationCorrection').forEach(b=>b.addEventListener('click',()=>window.openOnlineClassificationCorrection?.(b.dataset.correctId,b.dataset.correctTitle)));box.querySelectorAll('.onlineSupporting').forEach(b=>b.addEventListener('click',()=>document.querySelector(`#online article[data-online-article-id="${CSS.escape(String(b.dataset.supportId))}"] .onlineSupporting`)?.click()));box.querySelectorAll('.onlineIrrelevant').forEach(b=>b.addEventListener('click',()=>document.querySelector(`#online article[data-online-article-id="${CSS.escape(String(b.dataset.irrelId))}"] .onlineIrrelevant`)?.click()));article.appendChild(box);
  }
  function reset(root){
