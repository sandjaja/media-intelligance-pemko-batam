(()=>{
  ...
  function memberCard(m,articleEl){if(articleEl){const clone=articleEl.cloneNode(true);clone.style.display='';clone.removeAttribute('data-story-cluster-hidden');clone.removeAttribute('data-story-cluster-representative');clone.querySelector('.onlineStoryClusterBox')?.remove();clone.classList.add('rounded-xl','border','border-slate-800','bg-slate-950/60','p-3');return clone.outerHTML;}return `<div class="rounded-lg border border-slate-800 bg-slate-950/60 p-3"><div class="flex flex-wrap items-center justify-between gap-2"><span class="text-[10px] font-bold text-cyan-300">${esc(m.source_name||'-')}</span><span class="text-[10px] text-slate-500">${fmt(m.published_at)}</span></div><a href="${esc(m.url||'#')}" target="_blank" rel="noopener" class="block mt-1 text-xs text-slate-200 hover:text-cyan-300">${esc(m.title||'-')}</a><div class="text-[9px] text-slate-500 mt-1">#${esc(m.article_id)} · ${esc(m.similarity_type||'-')} · similarity ${Number(m.similarity_score||0).toFixed(2)}</div></div>`;}
