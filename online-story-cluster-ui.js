(()=>{
  ...
  function status(text,tone='slate'){
    ...
    if(!el){el=document.createElement('div');el.id='onlineStoryClusterStatus';root.prepend(el);}
  ...
  async function apply(){
    ...
    try{
      ...
      const multi=clusters.filter(c=>Number(c.member_count)>1);
      if(articles.size===0){status(`Story Cluster: ${clusters.length} cluster · ${multi.length} multi-publikasi · menunggu Feed Berita…`);scheduleRetry(400);return;}
