(()=>{
  ...
  async function runLatest(){
    ...
    try{
      ...
      showResult(results);const failed=results.filter(r=>r.error).length,inserted=results.reduce((n,r)=>n+Number(r.inserted||0),0);let clusterNote='';if(inserted>0){setState('Memperbarui story cluster...','fa-spinner');try{const response=await api('/online/story-clusters/rebuild',{method:'POST',body:JSON.stringify({days:7,limit:1000})});const clusterResult=response.result||{};clusterNote=` · ${Number(clusterResult.clusters||0)} cluster diperbarui`;showClusteringResult(clusterResult);}catch(e){clusterNote=' · cluster gagal diperbarui';}}window.toast?.(failed?`Selesai: ${inserted} berita baru, ${failed} sumber gagal${clusterNote}.`:`Selesai: ${inserted} berita baru disimpan${clusterNote}.`);document.getElementById('onlineRefresh')?.click();
