(()=>{
  ...
  async function runLatest(){
    ...
    try{
      ...
      showResult(results);const failed=results.filter(r=>r.error).length,inserted=results.reduce((n,r)=>n+Number(r.inserted||0),0);let clusterNote='';if(inserted>0){setState('Memperbarui story cluster...','fa-spinner');try{const response=await api('/online/story-clusters/incremental',{method:'POST',body:JSON.stringify({days:7,limit:200})});const clusterResult=response.result||{};clusterNote=` · ${Number(clusterResult.attached||0)} masuk kelompok, ${Number(clusterResult.created||0)} kelompok baru`;}catch(e){clusterNote=' · cluster gagal diperbarui';}}window.toast?.(failed?`Selesai: ${inserted} berita baru, ${failed} sumber gagal${clusterNote}.`:`Selesai: ${inserted} berita baru disimpan${clusterNote}.`);if(typeof window.renderOnlineMediaWorkspace==='function')await window.renderOnlineMediaWorkspace();else document.getElementById('onlineRefresh')?.click();
