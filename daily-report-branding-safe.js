(()=>{
 ...
 async function applyBranding(){
  ...
  inner.textContent='Daily Media Intelligence Report Pemko Batam';
  const header=inner.parentElement,logo=await preloadLogo();
  header.querySelectorAll('[data-pemko-report-logo]').forEach(n=>n.remove());
  ...
  const row=document.createElement('div');row.dataset.pemkoReportLogo='1';row.className='flex items-center gap-4 mb-2';
  row.innerHTML='<img alt="Logo Pemerintah Kota Batam" style="width:58px;height:70px;object-fit:contain;flex:none"><div class="min-w-0"><h1 class="text-2xl font-black leading-tight">Daily Media Intelligence Report Pemko Batam</h1></div>';
 ...
 window.addEventListener('media-intelligence-tab',e=>{if(e.detail==='reports')scheduleBranding()});document.addEventListener('click',e=>{if(e.target.closest?.('#reportBtn,#reportGenerate'))scheduleBranding()});
 document.addEventListener('click',async e=>{if(!e.target.closest?.('#reportPrint'))return;e.preventDefault();e.stopImmediatePropagation();ensurePrintStyle();await applyBranding();setTimeout(()=>window.print(),80)},true);
